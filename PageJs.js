  ...
  if (window.CanDesign == 'True') {
    ...
    if(timedata!=undefined && timedata2!= undefined){
      ...
      if(index >-1) {
        ...
        if($('#timeRemindbox').css('visibility') == 'hidden'){
              let count = 5
              $('.ellipse1').html(decodeURIComponent(curontentt.content))
              $('.emoji').attr('src','/images/Cartoon/'+curontentt.emoji+'.png')
              $('#timeRemindbox').css('visibility','visible')
              $('.ellipsebox').addClass('ellipseboxtrans')
              $('.emoji').addClass('emojitrans')
              $('.timeRemindclose').html('3S 关闭')
              inte = setInterval(function(){
                $('.timeRemindclose').html(count+'S 关闭')
                if(count == -1) {
                  clearInterval(inte)
                  $('#timeRemindbox').css('visibility','hidden')
                  $('.ellipsebox').removeClass('ellipseboxtrans')
                  $('.emoji').removeClass('emojitrans')
                  //count = 3
                  setTime = 1000
                }
                count--
              },1000)
              $('.timeRemindclose').click(function(){
                //count = 3
                $('#timeRemindbox').css('visibility','hidden')
                $('.ellipsebox').removeClass('ellipseboxtrans')
                $('.emoji').removeClass('emojitrans')
...
function resizeImg(){
	...
	for(var i = 0; i < imgs.length;i++){
		...
		if(src && src.indexOf("/comdata/") > -1 && src.indexOf("/comdata/resize/") == -1) {
			src = src.replace("/comdata/","/comdata/resize/");
			var pwidth = img.parent().width();
			if(!pwidth) pwidth = img.parent().parent().width(); //如果img的上级不是块元素，可能取出的宽度为0，这时尝试再从上级去取
			var width = 0;
			if (pwidth <= 2500 && pwidth > 0) width = 2500;
			if (pwidth <= 2000 && pwidth > 0) width = 2000;
			if (pwidth <= 1500 && pwidth > 0) width = 1500;
			if (pwidth <= 1200 && pwidth > 0) width = 1200;
			if (pwidth <= 1000 && pwidth > 0) width = 1000;
			if (pwidth <= 800 && pwidth > 0) width = 800;
			if (pwidth <= 600 && pwidth > 0) width = 600;
			if (pwidth <= 400 && pwidth > 0) width = 400;
			if (pwidth <= 200 && pwidth > 0) width = 200;
			if (pwidth <= 100 && pwidth > 0) width = 100;
			if (width >= 100)
			{
				src = src + "!"+width+"x00";
				img.attr('src',src);
...
document.addEventListener("DOMContentLoaded",function(){
	if(location.href.indexOf("resizeimg") > -1) resizeImg();
},false);
...
function delayload2(option){
	...
	if(getCookie("sid")) return;//登录后不执行延时加载
	//图片未加载时显示的图片
